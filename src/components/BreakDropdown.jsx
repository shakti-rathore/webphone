...
const BreakDropdown = ({ bridgeID, dispoWithBreak, selectedStatus }) => {
  ...
  const removeBreak = async () => {
    try {
      const response = await axios.post(`https://${window.location.origin}.iotcom.io/user/removebreakuser:${username}`);
    ...
    try {
      if (dispoWithBreak && breakType !== 'Break') {
        ...
        const dispositionResponse = await axios.post(
          `https://${window.location.origin}.iotcom.io/user/disposition${username}`,
          dispositionData,
          {
            headers: {
      ...
      const response = await axios.post(`https://${window.location.origin}.iotcom.io/user/breakuser:${username}`, {
        breakType,
      });
